...
// 后续带上每个物体独有的数据
void GfxBatchBuiltin::addObject(std::vector<float> &instanceData)
{
    if (instanceData.size() == 0)
    {
        throw std::runtime_error("GfxBatchBuiltin::addObject() instanceData size is 0!");
    }
