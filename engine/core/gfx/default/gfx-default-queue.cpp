...
GfxDefaultQueue::GfxDefaultQueue(GfxDefaultRenderer *renderer)
{
    this->_renderer = renderer;
}
void GfxDefaultQueue::init()
{
    this->_createBuffers();
    this->_createVertexBuffers();
}
void GfxDefaultQueue::_createBuffers()
{
    this->_createFramebuffers();
    this->_createCommandBuffers();
}
void GfxDefaultQueue::_createFramebuffers()
{
    GfxDefaultRenderPass *pass = this->_renderer->getRenderPass();
    if (pass == nullptr)
    {
    ...
    for (size_t i = 0; i < swapChainImageViews.size(); i++)
    {
        ...
        framebufferInfo.width = Gfx::_context->getSwapChainExtent().width;   // width和height用于指定帧缓冲的大小
        framebufferInfo.height = Gfx::_context->getSwapChainExtent().height; // 交换链图像都是单层，layers设置为1
        framebufferInfo.layers = 1;
        if (vkCreateFramebuffer(Gfx::_context->getVkDevice(), &framebufferInfo, nullptr, &this->_framebuffers[i]) != VK_SUCCESS)
        {
...
void GfxDefaultQueue::_createCommandBuffers()
{
    ...
    if (vkAllocateCommandBuffers(Gfx::_context->getVkDevice(), &allocInfo, this->_commandBuffers.data()) != VK_SUCCESS)
    {
...
void GfxDefaultQueue::_createVertexBuffers()
{
...
void GfxDefaultQueue::render(uint32_t imageIndex, std::vector<VkCommandBuffer> &commandBuffers, std::vector<std::string> &pipelineOutds)
{
    ...
    for (size_t i = 0; i < pipelineOutds.size(); i++)
    {
        ...
        try
        {
            GfxTexture *texture = Gfx::_textures.at(textureUuid);
            if (texture == nullptr)
            {
                continue;
            }
        ...
        {
            // std::cout << "[Gfx : GfxDefaultQueue]:: render: texture not found:" << textureUuid << std::endl;
            continue;
        }
    }
    // 渲染结束
...
void GfxDefaultQueue::_draw(uint32_t imageIndex, GfxTexture *texture, GfxDefaultPipeline *pipeline)
{
    ...
    _viewport.x = 0.0f;
    _viewport.y = 0.0f;
    _viewport.width = (float)Gfx::_context->getSwapChainExtent().width;
    _viewport.height = (float)Gfx::_context->getSwapChainExtent().height;
    _viewport.minDepth = 0.0f;
    _viewport.maxDepth = 1.0f;
    _scissor.offset = {0, 0};
    _scissor.extent = {(uint32_t)(Gfx::_context->getSwapChainExtent().width), (uint32_t)(Gfx::_context->getSwapChainExtent().height)};
...
void GfxDefaultQueue::_resetCommandBuffer(uint32_t imageIndex)
{
    vkResetCommandBuffer(this->_commandBuffers[imageIndex], 0);
}
void GfxDefaultQueue::_beginCommandBuffer(uint32_t imageIndex)
{
...
void GfxDefaultQueue::_beginRenderPass(uint32_t imageIndex)
{
...
void GfxDefaultQueue::_clear()
{
    for (auto framebuffer : this->_framebuffers)
    {
        vkDestroyFramebuffer(Gfx::_context->getVkDevice(), framebuffer, nullptr);
    }
    this->_framebuffers.clear();
    // 销毁命令缓冲（Command Buffers）
    if (!this->_commandBuffers.empty())
    {
        vkFreeCommandBuffers(Gfx::_context->getVkDevice(), Gfx::_context->getCommandPool(), static_cast<uint32_t>(_commandBuffers.size()), _commandBuffers.data());
        this->_commandBuffers.clear();
    }
}
void GfxDefaultQueue::_reset()
{
    this->_createBuffers();
}

GfxDefaultQueue::~GfxDefaultQueue()
{
}
...
//         {
//             vkUnmapMemory(Gfx::_context->getVkDevice(), this->_uniformBuffersMemory[i]);
//             this->_uniformBuffersMapped[i] = nullptr;
//         }
//         if (this->_uniformBuffers[i] != VK_NULL_HANDLE)
//         {
//             vkDestroyBuffer(Gfx::_context->getVkDevice(), this->_uniformBuffers[i], nullptr);
//             this->_uniformBuffers[i] = VK_NULL_HANDLE;
//         }
//         if (this->_uniformBuffersMemory[i] != VK_NULL_HANDLE)
//         {
//             vkFreeMemory(Gfx::_context->getVkDevice(), this->_uniformBuffersMemory[i], nullptr);
...
//         framebufferInfo.layers = 1;
//         if (vkCreateFramebuffer(Gfx::_context->getVkDevice(), &framebufferInfo, nullptr, &this->_queueFramebuffers[i]) != VK_SUCCESS)
//         {
...
//     if (vkAllocateCommandBuffers(Gfx::_context->getVkDevice(), &allocInfo, this->_queueCommandBuffers.data()) != VK_SUCCESS)
//     {
...
//     {
//         vkFreeCommandBuffers(Gfx::_context->getVkDevice(), this->_context->getCommandPool(), static_cast<uint32_t>(_queueCommandBuffers.size()), _queueCommandBuffers.data());
